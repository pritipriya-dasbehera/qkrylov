...
function lanczos_ground_state(
    ...
    if initial_vector !== nothing
        res = lanczos_lowest(H; n_eig=1, maxiter=maxiter, tol=tol, compute_eigenvectors=(return_state || compute_eigenvector), initial_vector=initial_vector)
        psi = (res.eigenvectors !== nothing && !isempty(res.eigenvectors)) ? res.eigenvectors[1] : nothing
        return GroundStateSolution(res.eigenvalues[1], psi, res.iterations, res.converged)
    end
